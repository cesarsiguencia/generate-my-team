...
Generator.prototype.managerPrompt = function () {
  inquirer.prompt([
...

    {
      ...
      this.managerInfo.name = answers.managername;
      this.managerInfo.id = answers.managerID;
      this.managerInfo.email = answers.manageremail;
      this.managerInfo.office = answers.manageroffice;
...
Generator.prototype.confirmAdd = function(){
  inquirer.prompt({
    type: 'confirm',
    name: 'addemployee',
...
Generator.prototype.addType = function(){
  inquirer.prompt({
    type: 'list',
    name: 'addcollague',
    message: 'Which type of employee would you like to add?',
    choices: ['Engineer','Intern','Generic']
  })
  .then(({ addcollague }) => {
    if(addcollague === 'Engineer'){
      this.addEngineer();
    } else if (addcollague === 'Intern'){
      this.addIntern();
    } else {
      inquirer.prompt([
        {
...

        {
          type: "text",
          name: 'employeeoffice',
          message: "What is the employee's office or what department do they work for?",
        }
        ...
        this.employeesArray.push(employeeAnswers)
        this.confirmAdd()
...
Generator.prototype.addEngineer = function (){
  inquirer.prompt([
    ...
    // console.log(answers);
    this.employeesArray.push(answers)
    this.confirmAdd();
  })
